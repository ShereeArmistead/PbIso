#' Calculates the model source mu
#'
#' This function calculates the model source Mu
#'
...
#'   CalcMu(t, x, y, T1, X1, Y1, U8U5, L5, L8)
#'
#' @examples
...
#'   CalcMu(2700, 33, 13.5, T1 = 4570, X1 = 9.307, Z1 = 31.23)
#'
CalcMu <- function(t, x, y, ..., T1 = 3700, X1 = 11.152, Y1 = 12.998, U8U5 = 137.88, L5 = 9.8485 * 10^-10, L8 = 1.55125 * 10^-10, model) {
  if(!missing(model)) {
    T1  = ifelse(is.null(model$T1), T1, model$T1)
    X1  = ifelse(is.null(model$X1), X1, model$X1)
    Y1  = ifelse(is.null(model$Y1), Y1, model$Y1)
  }
  ((exp(L5*t*10^6) - 1) / (U8U5 * (exp(L8*t*10^6) - 1)) * (X1 - x) + y - Y1) /
    ((exp(L5*T1*10^6) - exp(L5*t*10^6)) / U8U5 - (exp(L5*t*10^6) - 1) /
       (U8U5 * (exp(L8*t*10^6) - 1)) * (exp(L8*T1*10^6) - exp(L8*t*10^6)))
}
